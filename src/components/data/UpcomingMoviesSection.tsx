/**
 * Upcoming Movies Section - Real TMDb Data
 */

'use client';

import { useUpcomingMovies, usePrefetchMovie } from '@/lib/query';
...
export default function UpcomingMoviesSection() {
  const { data, isLoading, isError, error, refetch } = useUpcomingMovies(1);
  const prefetchMovie = usePrefetchMovie();

  if (isLoading) {
    return <MovieCarouselSkeleton title="Upcoming Releases" />;
  }
  ...
  if (isError) {
    return (
      <ErrorState
        ...
        message={error?.message || 'Failed to load upcoming movies'}
        onRetry={refetch}
      />
    );
  }
